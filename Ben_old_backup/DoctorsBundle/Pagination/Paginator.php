...
class Paginator implements IteratorAggregate
{
    ...
    {
        $uri = ((!empty($_SERVER['HTTPS']) && 'off' != $_SERVER['HTTPS'])
                    || 443 == $_SERVER['SERVER_PORT']) ? 'https' : 'http';
    ...
    private function prepareUrl()
    {
        ...
        if (0 == count($m)) {
            ...
            } elseif (false !== strpos($pagePattern, '/')) { //Friendly url : /page/123
                $segment = '';
                if (false !== strpos($url, '?')) {
